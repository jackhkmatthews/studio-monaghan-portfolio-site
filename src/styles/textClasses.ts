export const textClasses = {
  wordMark: "font-serif text-xl md:text-2xl font-bold leading-none",
  h1: "font-serif text-xl md:text-2xl font-bold",
  h2: "font-serif text-lg italic",
  h3: "font-serif text-base font-bold",
  navLink: "font-serif text-base md:text-lg",
  body: "font-serif text-base",
  portableText: "prose lg:prose-lg max-w-[50ch] font-serif",
  portableTextFooter: "prose-sm font-serif leading-none",
} as const;
